CREATE TABLE Utilizador (
  IDU VARCHAR(10) NOT NULL, -- Formato XX_Nome, onde XX é o prefixo do tipo de utilizador, gerado automaticamente (trigger)
...
CREATE TABLE Reserva (
  IDR VARCHAR(8) NOT NULL, -- Formato yyyySSSS, onde yyyy é o ano e SSSS é sequencial
...
CREATE TABLE Historico_Prioridade (
  ...
  IDU VARCHAR(10) NOT NULL, -- Referência ao utilizador
  PRIMARY KEY (IDHP),
  ...
  CHECK (prioridade_nova IN ('Máxima', 'Acima da Média', 'Média', 'Abaixo da Média', 'Mínima'))
);
...
CREATE TABLE Historico_Estado (
  ...
  IDR VARCHAR(8) NOT NULL, -- Referência à reserva
  PRIMARY KEY (IDHE),
  FOREIGN KEY (IDR) REFERENCES Reserva(IDR),
